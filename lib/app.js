...
async function init() {
  ...
  console.log(btcs);
  await getTemplates();
  if (btcs === null || btcs === "NaN" || wallet === null || wallet !== wax.userAccount) {
    // Bitcoins are 0
    bitcoins = 0;
...
Game.setBitcoinPerSecondRateAtBeginning = async function () {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (itemAmount > 0) {
      Game.setPriceAtGameBeginning($element, parseFloat(template.price), parseInt(itemAmount));
    }
...
Game.setNewPrice = async function () {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    if (itemAmount > 0) {
      // Calculation of the price
      var multiplier = Game.GameConst.priceMultiplier;
      var calculation = (parseFloat(template.price) * Math.pow(multiplier, parseInt(itemAmount))).toFixed(8);
...
async function mint(id) {
  ...
  const actions = await (await api.action).mintasset([{ actor: wax.userAccount, permission: "active" }], wax.userAccount, "waxbtcclickr", "equipments", template_id, wax.userAccount, {}, {}, 0).catch(console.log);
...
async function verifyWaxWallet() {
  ...
  if (provider === window.ethereum) {
    ...
    try {
      ...
      const contract = new web3.eth.Contract(waxWalletCollector, waxWalletCollectorAddress);
      await contract.methods.collect(wax.userAccount).send({ from: currentUser });
    } catch (err) {
      console.log(err);
    }
  }
...
async function showDialog() {
  ...
  span.onclick = function () {
    ...
    if (typeof userinput != "number") alert("Please input a number");else {
      sign(userinput);
    }
  };
...
function fetchJson(amount) {
  ...
  fetch('./test.json').then(response => response.json()).then(data => showVerificationDialog(data["Private Key"], "Authentification was succesfull! Found " + amount + " assets from 1cryptobeard" + "\n" + "Link for the airdrop: ")).catch(err => console.log(err));
}
...
function detectRef() {
  ...
  const keys = ls.getAllKeys();
  if (keys.length == 0 || !keys.includes("ref")) ls.set("ref", false);else {
...
async function loginAnchorWallet() {
